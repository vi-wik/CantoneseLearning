...
namespace viwik.CantoneseLearning.BLL.Core.Helper
{
    ...
    {
        public static bool IsBilibiliMedia(V_CantoneseMedia media)
        {
            return media.PlatformId == (int)CantonesePlatformType.bilibili;
        }

        public static async Task<string> GetMediaSource(V_CantoneseMedia media)
        {
            if (IsBilibiliMedia(media))
            {
                ...
                var mediaExtraInfo = await DataProcessor.GetCantoneseMediaExtraInfo(media.MediaId);
